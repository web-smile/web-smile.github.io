...
{
	options=options || {};//定义空{},为了防止报错
	var duration=options.duration || 1000;//默认时间
	var easing=options.easing || Tween.Linear; //默认运动类型，是一个函数体
	...
	obj.timer=setInterval(function (){
		...
		{
			clearInterval(obj.timer);
			options.complete && options.complete();// 可选的结束函数
		}
	}, 30);
}
